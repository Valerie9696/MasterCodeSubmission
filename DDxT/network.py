...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        lnx = self.ln(x)
        attention, weights = self.attn(query=lnx, key=lnx, value=lnx, mask=mask, look_ahead_mask=None)
        attention = self.dropout(attention)
        x = x + attention

        lnx = self.ln(x)
        feed_forward = self.mlp(lnx)
        return x + feed_forward, weights
...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, encoder_output, en_mask, de_mask):
        ...
        attention, weights = self.attn_mask(query=de_in, key=de_in, value=de_in, mask=de_mask,
                                   look_ahead_mask=self.causal_mask(de_in))
        attention = self.dropout(attention)
        x = decoder_input + attention

        en_out = self.ln(encoder_output)
        attention, weights = self.attn_en_de(query=x, key=en_out, value=en_out, mask=en_mask, look_ahead_mask=None)
        attention = self.dropout(attention)
        x = x + attention
        ...
        attn_probs = weights.view(bsz, tgt_len, -1)
