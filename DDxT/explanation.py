...
def sort_evidences(pathology, names, evidence_codes, weights):
    ...
    for i, ddx in enumerate(names):
        ...
        topk_weights = top_weights_full[:threshold]
...
def set_threshold(probabilities, min_out, max_out, evidence_names = []):
    ...
    if evidence_names:
        ...
        plt.plot(x, probabilities.cpu(), label='Probabilities', color='blue', marker='o')
        if knee_point:
            plt.scatter(knee_point, probabilities.cpu()[knee_point - 1], color='orange', label='Knee Point', zorder=5)
        plt.xticks(ticks=x, labels=evidence_names, rotation=45, ha='right')
        plt.xlabel('Evidence Names')
        plt.ylabel('Norm Values')
        plt.title('Probabilities with Knee Point')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
