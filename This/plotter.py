...
def make_pie_chart(pathology_name, values, path):
        """
        Make the pie charts for the explanation charts and save them.
        :param pathology_name: Name of the pathology
        :param values: values for the pie chart
...
def make_bar_chart(pathology_name, values, path):
    """
    Make the bar charts for the explanation charts and save them.
    :param pathology_name: Name of pathology
...
def read_file(path, mode, placeholder):
    """
    Read files of bar charts and pie charts.
    :param path: path of file
    :param mode: mode
    :param placeholder: in case the file does not exist, return a white placeholder file called 'placeholder.png'
    :return:
    """
    try:
...
def plot_ddx_attention_graph(en_in, de_in, input_vocab, output_vocab, en_attention_weights, de_attention_weights, pathology, counter):
    ...
    #dot.attr('node', width='10', height='10', fixedsize='true', shape='ellipse', margin='0.2,0.2')

    #dot.attr(smoothType='curved')

    # use shades of blue to color the nodes
...
def plot_path_ddx_attention_graphs(en_in, de_in, input_vocab, output_vocab, de_attention_weights, pathology, counter):
    ...
    for idx, token in enumerate(de_in):
        ...
        if ddx not in ['<bos>', '<eos>', '<pad>']:
            ddx_nodes.append(ddx)
            color_map[ddx] = de_attention_weights[idx].mean().item()
    with open(os.path.join(cfg.DATA_DIR, cfg.VALUES), 'rb') as pickle_file:
        value_meanings = pkl.load(pickle_file)
    antecedents = []
    symptoms = []
...
def assemble_chart(id, title, pathology_name, ddx_names, patient_path):
    """
    Put everything together. Get the graph, the text, and the pie/bar charts per patient and assemble everything
    in a html file.
    :param id: patient id
    :param title:
    :param pathology_name: name of pathology
    :param ddx_names: names of differential diagnoses
    :param patient_path:
    :return:
    """
    ...
    .card-text {
        ...
        //margin: 0 auto;
        margin-bottom: 12px;
        min-height: 650px; // too big for some, an upper bound
        text-align:left;
    }
    .centered-image {
        margin: auto;
